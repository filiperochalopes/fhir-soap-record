...
import type { GenericPluginCardProps } from "~/lib/soap-plugins/types";
...
function AttachmentRow(props: {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-[color:var(--panel-border)] bg-white/30 px-3 py-2 text-sm dark:bg-slate-900/30">
...
export function AttachmentsCard(props: GenericPluginCardProps) {
  ...
  useEffect(() => {
    if (uploadFetcher.state === "idle" && uploadFetcher.data && !uploadFetcher.data.error) {
      listFetcher.load(attachmentsUrl);
      setSelectedFile("");
      if (inputRef.current) {
  ...
  const data = uploadFetcher.data && !uploadFetcher.data.error ? uploadFetcher.data : listFetcher.data;
  ...
  return (
    <PluginCard
      description="Anexa arquivos ao rascunho atual e vincula ao atendimento quando a nota for salva."
      label="Plugin"
      title="Anexos"
      tone="violet"
    >
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button className="button-secondary" disabled={isUploading} type="submit">
      ...
      <div className="space-y-2">
        ...
        {data?.draft.length ? (
          ...
          <p className="text-sm text-[color:var(--muted)]">Nenhum anexo no rascunho.</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {data?.attached.length ? (
          ...
          <p className="text-sm text-[color:var(--muted)]">Nenhum anexo salvo.</p>
        )}
      </div>
    </PluginCard>
  );
